...
/// Whole-entry last-writer-wins on the Lamport clock (the pre-0.6 behaviour).
...
impl InboundApplier for AdapterRouter {
    fn apply_inbound(&self, state: &mut State, entries: Vec<Entry>) -> Result<usize> {
        ...
        for entry in entries {
            match adapters
                .iter()
                .position(|adapter| adapter.owns_key(&entry.key))
            {
...
mod tests {
    ...
    fn adapters_report_owned_keys() {
        ...
        let logical =
            LogicalAdapterWrapper::new(Arc::new(InMemoryLogicalAdapter::new("logical", "app")));
    ...
    fn router_applies_field_merges_and_falls_back_to_lww() {
        ...
        assert!(router
            .register(Arc::new(JsonFileAdapter::new("file", "x.json")))
            .is_err());
