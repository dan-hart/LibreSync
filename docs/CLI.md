...
The CLI is a thin wrapper around the core library. It covers the full flow to configure adapters (JSON or SQLite), discover devices, pair, refresh, manage backups, and rotate or export keys.
...
- `device set-address`: store a manual address for a paired device.
- `unpair`: revoke pairing with a device (removes from allowlist).
- `listen`: run the device listener and advertise on LAN.
- `refresh`: refresh a selected adapter with a paired device (use `--all` to refresh all paired devices, `--all-adapters` to refresh all selected adapters).
- `watch`: watch selected adapter files and refresh all paired devices (auto-starts a listener by default).
...
- `key rotate-device`: rotate device TLS keys and fingerprint (remote devices must re-pair).
...
- `devices`: map of paired devices keyed by device ID (includes last seen address and fingerprint).
...
7. Run `refresh` whenever you want to refresh. The same address/device-ID selection rules apply. Use `refresh --all` to refresh every paired device, or `--all-adapters` to refresh all selected adapters.
8. Run `watch` to refresh all paired devices on local changes and on a periodic interval (supports multiple adapters).
9. Run `status` to see connected devices and last seen addresses.
10. Run `diagnose` to verify config, keys, and storage before pairing.
...
- `key rotate` clears the allowlist by default; re-pair devices after rotation.
- `key rotate-device` changes the local fingerprint; remote devices must re-pair.
